...
pars=c( "I0A", "I0B", "fB", "I0Tot", 
        "probICU", "probDICU",
        "alpha" , "muHosp",  "muDICU", "phi", "I0Tot", "fD", "beta[1,1]", "beta[3,1]", "beta[4,1]", "beta[8,1]")

print(fit_stan, pars = pars)
print(fit_stan)
stan_dens(fit_stan, pars = pars, separate_chains = T)
...
hist(summary(fit_stan)$summary[,"Rhat"])
...
length(dateSeq)
data <- data.table(
  cases = c(casAAgg + casBAgg, casAAgg, casBAgg), 
  rollMean = c(zoo::rollmean(casAAgg+casBAgg, 7, na.pad = T), zoo::rollmean(casAAgg, 7, na.pad = T), zoo::rollmean(casBAgg, 7, na.pad = T)),
  types = c(rep("Total", length(casAAgg)), rep("Alpha", length(casAAgg)), rep("Delta", length(casAAgg))),
  dates = rep(dateSeq, 3)
)
...
dataHosp <- data.table(
  dates = dateSeq,
  hosp = hospAgg,
  hospRollMean = zoo::rollmean(hospAgg, 7, na.pad = T)
)

p1 <- ggplot(data, aes(x = dates)) +
  geom_vline(xintercept = as.Date("2021-05-09"), linetype = "dashed") +
  annotate(geom = 'text', label = 'end state of alarm', x = as.Date("2021-05-14"), y = 5500,  angle = 90) +
  geom_vline(xintercept = as.Date("2021-07-15"), linetype = "dashed") +
  annotate(geom = 'text', label = 'curfew', x = as.Date("2021-07-25"), y = 8000,  angle = 90) +
  ...
  annotate("rect", fill = "grey", alpha = 0.7, 
           ...
           ymin = -Inf, ymax = Inf) +
  geom_point(aes(y = cases, col = types), shape = 5, size = 1.0, stroke = .8, alpha = 0.4) +
  scale_color_manual(values = c( c_posterior, "#2b8cbe", "black")) +
  geom_line(aes(y = rollMean, col = types), size = 1.1) +
  labs(color = "") +
  xlab(" ") +
  ylab("Incidence") +
...
p2 <- ggplot(dataHosp, aes(x = dates)) +
  geom_vline(xintercept = as.Date("2021-05-09"), linetype = "dashed") +
  geom_vline(xintercept = as.Date("2021-07-15"), linetype = "dashed") +
  annotate("rect", fill = "grey", alpha = 0.4, 
           xmin = as.Date("2021-06-20"), xmax =  as.Date("2021-07-09"),
  ...
  annotate("rect", fill = "grey", alpha = 0.7, 
           xmin = as.Date("2021-06-23"), xmax =  as.Date("2021-06-27"),
           ymin = -Inf, ymax = Inf) +
  geom_point(aes(y = hospAgg), shape = 5, size = 1.0, stroke = .8, alpha = 0.4) +
  #scale_color_manual(values = c( c_posterior, "#2b8cbe", "black")) +
  geom_line(aes(y = hospRollMean), size = 1.1) +
  labs(color = "") +
  xlab(" ") +
...
p3 <- plot_grid(p1, p2, ncol = 1, align = "v", labels = c("A", "B"), rel_heights = c(0.85, 1.0))

casesCat.dt <- fread("catalunya_diari_total_pob.csv",header=TRUE)
...
vacCat.dt$GRUP_EDAT <- rep(ages, each = length(unique(vacCat.dt$DATA)))
vacCat.dtAgg <- as.data.table(casesCat.dt  |>
                                dplyr::group_by(DATA) |>
                                dplyr::summarise(vac1 = sum(VACUNATS_DOSI_1),
                                    vac2 = sum(VACUNATS_DOSI_2)) |>
                                dplyr::mutate(vac1Rel = cumsum(vac1)/sum(N)) |>
                                dplyr::mutate(vac2Rel = cumsum(vac2)/sum(N)) |>
                                dplyr::filter(DATA > startDate-n_pre-n_vacc))

vacCat.dtAgg$GRUP_EDAT <- rep("Total", length(vacCat.dtAgg$DATA))

vacCat.dt$N <- rep(N, each = length(unique(vacCat.dt$DATA)))

vacCat.dt <- vacCat.dt |>
  dplyr::ungroup() |>
  dplyr::group_by(GRUP_EDAT) |>
  dplyr::mutate(vac1Rel = cumsum(vac1)/N) |>
  dplyr::mutate(vac2Rel = cumsum(vac2)/N) |>
  dplyr::filter(DATA > startDate-n_pre-n_vacc)

vacCat.dt <- rbind(vacCat.dt, vacCat.dtAgg)

vacCat.dt$vac1 <- NULL
vacCat.dt$vac2 <- NULL

vacCat.dt$N <- NULL
colnames(vacCat.dt) <- c("GRUP_EDAT", "DATA", "One dose", "Two doses")
vacCat.dt <- reshape2::melt(vacCat.dt, id.vars=c("GRUP_EDAT", "DATA"))

vacCat.dt <- vacCat.dt[vacCat.dt$GRUP_EDAT != "0-9",]
p4 <- ggplot(vacCat.dt) +
  geom_vline(xintercept = as.Date("2021-05-09"), linetype = "dashed") +
  geom_vline(xintercept = as.Date("2021-07-15"), linetype = "dashed") +
  annotate("rect", fill = "grey", alpha = 0.4, 
  ...
  annotate("rect", fill = "grey", alpha = 0.7, 
           xmin = as.Date("2021-06-23"), xmax =  as.Date("2021-06-27"),
           ymin = -Inf, ymax = Inf) +
  geom_line(aes(x = DATA, y = value, col = GRUP_EDAT), size = 1.3) +
  facet_wrap(~variable, ncol = 1) +
  labs(color = " ") +
  ylab(" ") +
  scale_y_continuous(labels = scales::percent) +
  scale_colour_manual(values = c(RColorBrewer::brewer.pal(9,"Spectral")[c(1:4,6:9)], "grey40")) +
...
p <- plot_grid(p3, p4, labels = c("", "C"), label_size = 12)
save_plot("plots/panel0.pdf", p,
          base_height =4.3, 
          base_width = 8.25)
...
colnames(smr_pred) <- make.names(colnames(smr_pred)) # to remove % in the col names
smr_pred <- smr_pred[(n_pre+1):length(smr_pred$mean),]
smr_pred$date <- seq(from = startDate, by = "day", length = length(smr_pred$X2.5.))
...
mean(abs(diff(Dt1$Rt)))
Dt2 <- Dt[Dt$dates < t1 || Dt$dates > t2,]
mean(abs(diff(Dt2$Rt)))
...
p1 <- ggplot() +
  ...
  #annotate("rect", fill = "lightblue", alpha = 0.4, 
           #xmin = as.Date("2021-07-15"), xmax =  as.Date("2021-08-19"),
           #ymin = -Inf, ymax = Inf) +
...
smr_pred <- cbind(as.data.frame(summary(
  fit_stan, pars = "predCasesAge", probs = c(0.025, 0.5, 0.975))$summary))
colnames(smr_pred) <- make.names(colnames(smr_pred))
smr_pred$ages <- rep(ageGroups, each = n_days)
dateVector <- rep(as.Date("2020-05-21"), M*n_days)
dateVector[1] = as.Date("2020-01-05")
dateSeq <- seq(from = startDate, by = "day", length = n_days)
casesVector = cases[1,]
index <- 1
for(j in 2:M){
  casesVector <- c(casesVector, cases[j,])
}
smr_pred$cases <- casesVector
smr_pred$date <- rep(dateSeq, M)
...
p <- ggplot(smr_pred) +
  geom_point(aes(x = date, y = cases),
             shape = 21, colour = col, fill = f, size = s, stroke = stro, alpha = alph) +
  geom_ribbon(aes(x = date, ymax = X2.5., ymin = X97.5.), fill = c_posterior, alpha = 0.35) +
  geom_line(aes(x = date, y = X50.),color = c_posterior) +
  facet_wrap(~ages, scale = "free_y") +
  ylab("Incidence") +
  xlab("") +
  theme_minimal(base_size = bs) +
  theme(panel.grid.major = element_blank(), 
        ...
        strip.background = element_blank(),
        axis.ticks = element_line(size = 0.5, color="black"))
save_plot("plots/supp_CasesAges.pdf", p,
          base_height = 5.0,
          base_width = 8.25)


smr_pred <- cbind(as.data.frame(summary(
  fit_stan, pars = "hospPredAge", probs = c(0.025, 0.5, 0.975))$summary))
colnames(smr_pred) <- make.names(colnames(smr_pred))
smr_pred$ages <- rep(ageGroups, each = n_days)
dateVector <- rep(as.Date("2020-05-21"), M*n_days)
dateVector[1] = as.Date("2020-01-05")
...
save_plot("plots/supp_hospAges.pdf", p,
          base_height = 5.0,
          base_width = 8.25)
...
p <- ggplot(smr_pred, mapping = aes(x = date)) +
  geom_vline(xintercept = as.Date("2021-05-09"), linetype = "dashed") +
  geom_vline(xintercept = as.Date("2021-07-15"), linetype = "dashed") +
  annotate("rect", fill = "grey", alpha = 0.4, 
           xmin = as.Date("2021-06-20"), xmax =  as.Date("2021-07-09"),
  ...
  theme(panel.grid.major = element_blank(), 
        ...
        axis.ticks = element_line(size = 0.5, color="black"),
        axis.text.x = element_blank())
save_plot("plots/supp_repNumber.pdf", p,
          base_height = 5.0,
          base_width = 8.25)
...
p <- ggplot(smr_pred, mapping = aes(x = date)) +
  ...
  theme(panel.grid.major = element_blank(), 
        ...
        axis.ticks = element_line(size = 0.5, color="black"),
        axis.text.x = element_blank())
save_plot("plots/supp_interactionRate.pdf", p,
          base_height = 5.0,
          base_width = 8.25)
